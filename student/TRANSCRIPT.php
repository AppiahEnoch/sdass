...
//$_SESSION['admission_number']="J12023-4-5";
...
// SELECT term_year, school_class_id, user_id 
// FROM assessment 
// WHERE admission_number = $_SESSION['admission_number']
$admission_number = $_SESSION['admission_number'];
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        $studentData[] = $row;
        //echo  $row['term_year']." ".$row['class_id']." ".$row['user_id']."<br>";
...
$admission_number=$_SESSION['admission_number'];
 ...
 $pdf->Output('F', '../report/'. $admission_number. '.pdf');

 echo '../report/'. $admission_number. '.pdf';

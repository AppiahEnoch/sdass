...
function get_remarks_template() {
  $.ajax({
    ...
    error: function() {
      alert('Error while getting remarks template.');
    }
  });
}
      ...
      success: function (data, status) {
        aeLoading()
          ...
          function createRow(student, property, trackFunction) {
              ...
              let row = $('<tr><td title="' + student.admission_number + '">' + truncatedAdmissionNumber + '</td><td title="' + student.fullName + '">' + truncatedName + '</td><td contenteditable="true">' + student[property] + '</td></tr>');

...
function uploadAssessmentData() {
    ...
    $.ajax({
      ...
      success: function (data, status) {
        ...
        // reset
        $("#teacher_submit_assessment_form")[0].reset();
        aeLoading()
...
$(document).ready(function() {
  ...
  //  alert(global_academic_term)
   
    var value = $("#teacher_search_input").val().toLowerCase();
    var termFilterOn = $("#teacher_switch_term_assessment_list").is(':checked');

    $(".teacher_class_assessment_list_card").filter(function() {
      var cardHeaderText = $(this).find(".card-header").text().toLowerCase();
      var matchesTerm = cardHeaderText.includes(global_academic_term.toLowerCase());
      var matchesSearch = cardHeaderText.indexOf(value) > -1;
      
      if (termFilterOn) {
        $(this).toggle(matchesTerm && (value === "" || matchesSearch));
      } else {
        $(this).toggle(value === "" || matchesSearch);
      }
    });
  }

  $("#teacher_search_input").on("keyup", filterCards);
  $("#teacher_switch_term_assessment_list").on("change", filterCards);
});

...
$pdf->AddPage();
 ...
 // Get the examination id dynamically from the database
...
if (stripos($term, 'Third Term') !== false) {
    ...
    $term_year = $conn->real_escape_string($_SESSION['current_term']);
    $class_id = $conn->real_escape_string($_SESSION['userClass']);
...
$stmt = $conn->prepare($sql);
    ...
    if(\AE\AE::isEmpty($attendance)) {
        $attendance =0;

    }
    if(\AE\AE::isEmpty($total_attendance)) {
     $total_attendance=0;
